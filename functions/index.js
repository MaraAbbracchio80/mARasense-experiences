...
app.use((req, res, next) => {
  ...
  res.set(
    ...
    "Content-Type,Authorization,x-goog-meta-firebasestoragedownloadtokens"
  );
...
const STORAGE_BUCKET =
  process.env.STORAGE_BUCKET || "marasenseexperiences-ar.firebasestorage.app";
...
function normalizeExperiencePayload(body) {
  ...
  const rawContents = Array.isArray(body.contents) && body.contents.length > 0
    ? body.contents
    : [{ id: 0, label: "Contenuto 1", targetId: 0, file: contentFile, kind: "video" }];
  const contents = rawContents.map((content, index) => ({
    ...content,
    id: Number.isInteger(content.id) ? content.id : index,
    targetIds: normalizeTargetIds(content.targetIds, content.targetId ?? index),
    targetId: Number.isInteger(content.targetId)
      ? content.targetId
      : normalizeTargetIds(content.targetIds, index)[0]
  }));
...
function validateExperience(data) {
  ...
  for (const content of data.contents) {
    if (!isValidAssetReference(content.file)) {
      return `File contenuto non valido: ${content.file}`;
    }
    if (content.audioFile && !isValidAssetReference(content.audioFile)) {
      return `File audio non valido: ${content.audioFile}`;
    }
    const targetIds = normalizeTargetIds(content.targetIds, content.targetId ?? 0);
    if (targetIds.length === 0) {
      return "Ogni contenuto deve avere almeno un target collegato.";
    }
  }
...
function buildConfig(data, folder, publicUrl, previousConfig) {
  const previousMeta = previousConfig && previousConfig.meta ? previousConfig.meta : {};
  const createdAt = previousMeta.createdAt || new Date().toISOString();
  const updatedAt = new Date().toISOString();

  const normalizedContents = data.contents.map((content, index) => {
    const targetIds = normalizeTargetIds(content.targetIds, content.targetId ?? index);
    return {
      id: Number.isInteger(content.id) ? content.id : index,
      ...
      audioOriginalFile: content.audioOriginalFile || ""
    };
  });

  const flattenedTargets = normalizedContents.flatMap((content, index) =>
  ...
  const firstContent = normalizedContents[0] || {
  ...
  return {
    ...
    multiTarget:
      ...
      normalizedContents.some((content) => content.targetIds.length > 1),
...
app.post("/getUploadUrl", async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ok: true,
      uploadUrl,
      downloadUrl: storageDownloadUrl(rawPath, token),
      token,
      path: rawPath
    });
  } catch (error) {
    ...
    return res.status(500).json({
      ok: false,
      error: "Errore creazione URL upload.",
...
app.post("/createManagedExperience", async (req, res) => {
  try {
    ...
    const existingConfigFile = await githubGetFile(configPath);
    const previousConfig = existingConfigFile
      ? JSON.parse(decodeBase64(existingConfigFile.content))
      : null;

    const config = buildConfig(data, folder, publicUrl, previousConfig);
    ...
    await githubPutFileText(
      ...
      existingConfigFile ? existingConfigFile.sha : undefined
    );
